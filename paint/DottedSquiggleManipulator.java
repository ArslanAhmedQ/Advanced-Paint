 ...
 */
public class DottedSquiggleManipulator implements ShapeManipulatorStrategy{
	/**
	 * it constructs the dotted squiggle, while still dragging is in progress it would delete the previous dotted 
	 * squiggles  
	 * 
	...
	public void Construct(String type, int x0, int y0, int xf, int yf,ArrayList<Point> points, Color color, boolean fill, int thickness, PaintModel model, boolean dragOccurred, MouseEvent e){
		...
		model.addShape(new DottedSquiggle("DottedSquiggle",points, color, thickness));
	}
}
